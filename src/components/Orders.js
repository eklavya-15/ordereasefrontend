        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {orders && orders.map((order, index) => 
              ...
              <div className="text-right mt-4">
                ...
                <p className="text-lg font-semibold text-gray-800">Total: ₹{order.amount}</p>
                <button onClick={() => handleReorder(order.items)} className="mt-2 px-4 py-2 bg-green-700 text-white rounded-lg hover:bg-green-800 transition duration-300">
                  Reorder
                </button>
              </div>
              </div>)
